...
from PyQt5.QtWidgets import (
    QMainWindow, QApplication, QWidget, QLabel, QPushButton, QVBoxLayout, QHBoxLayout,
    ...
    QSizePolicy, QDockWidget, QSplitter, QSpinBox
)
...
class ShapeDesignerMainWindow(QMainWindow):
    ...
    def send_layout(self):
        ...
        info = f"[发送至PLC {self.ip_input.text()}:{self.port_input.text()}]\n{json_data}\n"
    ...
    def load_config(self):
        config_path = r'config\xin.json'
        items = self.scene.mapper.load_from_json(config_path)
        for item in items:
